...
export const useAuthService = (): UseAuthApi => {
  ...
  const login = async (username: string, password: string): Promise<void> => {
    try {
      ...
      if (axios.isAxiosError(err)) {
        throw new ServiceError(ERROR_CODE.AUTH_SERVICE_ERROR,`Logging failed, ${err.message}`, err.response?.data.message);
      }
  ...
  const register = async (username: string, password: string): Promise<void> => {
    try {
      ...
      if (axios.isAxiosError(err)) {
        throw new ServiceError(ERROR_CODE.AUTH_SERVICE_ERROR,`Registration failed, ${err.message}`, err.response?.data.message);
      }
      console.log('Unexpected error', err);
    }
  };
